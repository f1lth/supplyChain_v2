...
contract Chain is AccessControl{
    ...
    function produceModularPart(
        ...
        /**
        1) Chasis - cost = 3 metal

        2) Motor and Transmission - cost 2 metal, 1 plastic
        ...
        if productCode is 1:
            try:
                pop 3x metal from the raw queue somehow, 
                // will either linear search array for first 3 instances of metal
                // and if we get to end of array without required resources
                /// we wasted a lot of comp power, but we return failure

                // alternative idea, we have separate arrays for each raw type
            except:
                return failure
        
        elif productCode is 2:
            same shit
        .
        .
        .
